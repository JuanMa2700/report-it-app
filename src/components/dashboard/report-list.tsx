...
import { STATUS_LABELS, URGENCY_LABELS, ALL_STATUSES, ALL_URGENCIES } from '@/lib/constants'
  ...
  return (
    <div className="flex h-full flex-col rounded-lg border bg-background">
      <div className="flex shrink-0 gap-2 border-b p-3">
        <Select value={statusFilter} onValueChange={(v) => onStatusChange(v as ReportStatus | 'ALL')}>
        ...
        <Select value={urgencyFilter} onValueChange={(v) => onUrgencyChange(v as Urgency | 'ALL')}>
          <SelectTrigger className="h-8 text-xs">
      ...
      <div className="min-h-0 flex-1">
        <ScrollArea className="h-full">
          <div className="space-y-2 p-3">
            {reports.length === 0 ? (
              <p className="py-8 text-center text-sm text-muted-foreground">No hay reportes</p>
            ) : (
              reports.map((report) => (
                <ReportListItem
                  key={report.id}
