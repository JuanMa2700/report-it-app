...
const MEDIA_CONFIG: Record<MediaType, { icon: typeof Image; label: string; color: string; bg: string }> = {
  PHOTO: { icon: Image, label: 'Foto', color: 'text-blue-600', bg: 'bg-blue-50' },
  VIDEO: { icon: Video, label: 'Video', color: 'text-purple-600', bg: 'bg-purple-50' },
  AUDIO: { icon: Mic, label: 'Audio', color: 'text-amber-600', bg: 'bg-amber-50' },
}
...
function MediaItem({ media }: { media: ReportMedia }) {
  ...
  async function handleLoad() {
    ...
    setIsLoading(true)
    try {
      const { data } = await api.get<{ url: string }>(`/uploads/media/${media.id}`)
      ...
      {mediaUrl ? (
        <div className="relative">
          <button
            ...
            className="absolute right-1 top-1 z-10 rounded-full bg-black/50 p-0.5 text-white hover:bg-black/70"
          >
            <X className="h-3.5 w-3.5" />
          </button>
          {media.type === 'PHOTO' && (
            <a href={mediaUrl} target="_blank" rel="noopener noreferrer">
              <img src={mediaUrl} alt="Evidencia" className="w-full object-contain" />
            </a>
          )}
          {media.type === 'VIDEO' && (
